     ...
     dxl_cnt_(2)
{
  ...
  wheel_separation_ = node_handle_.param<float>("wheel_separation", 0.2760);
  wheel_radius_     = node_handle_.param<float>("wheel_radius", 0.0325);
...
bool VelocityControl::calcOdometry(const ros::Time &time)
{
  ...
  theta = (wheel_radius_ * (wheel_r - wheel_l) / wheel_separation_) / 2.0;  
